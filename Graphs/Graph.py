...
def buildGraphWithList(edges,n):
    ...
    for edge in edges:
        ...
        des = edge[1]
        wt = edge[2]
        graph[src-1].append([src,des,wt])
        graph[des-1].append([des,src,wt])
...
def MultiPurpose(graph,src,des,psf,visited,wsf):
    global longestPath,shortestPath,maximumWeight,minimumWeight
    if src == des:
        psf+=str(src)
        if len(psf) > len(longestPath):
            longestPath = psf
        if len(psf) < len(shortestPath):
            shortestPath=psf
...
# MultiPurpose(graph,1,8,"",[False for i in range(n+1)],99)
